...
### 🍎 macOS 사용자
맥의 경우 2가지 경우가 있습니다.
상황에 따라 다르기 때문에 적절한 상태에서 적용하시면 됩니다.

#### 경우 1️⃣
먼저 다운로드 받은 곳에서 프로그램을 더블클릭 하고 나면 다음과 같은 경고창이 나옵니다.
바로 휴지통으로 들어가려고 하죠.
![Mac 보안 문구](resource/Mac_task1.png)
이 경우에는 다음과 같이 진행하시면 됩니다.
1. 취소를 먼저 누르신 후 터미널을 켭니다.
2. `xattr -cr ~/Downloads/VProCouponManager.app` 를 붙여넣어서 실행해주세요
	1. 만약 경로가 다르다면 경로 수정해 주셔야 합니다.
3. 다시 실행하면 프로그램이 실행됩니다.
